...
void Node::getForce(Eigen::VectorXf& vector) const
{
    ...
    Eigen::Vector3f force = Eigen::Vector3f(0.0f, -9.81f, 0.0f) * _mass + damping;
...
void Node::getForceJacobian(std::vector<Eigen::Triplet<float>>& dFdx, std::vector<Eigen::Triplet<float>>& dFdv) const
{
...
void Node::fixMatrix(Eigen::SparseMatrix<float>& matrix) const
{
    if (_fixed) {
        ...
        matrix.prune([d](const Eigen::Index& row, const Eigen::Index& col, const float& value) {
            if (row == d && col == d) {
                return true;
            }

            return (d < row || d + 3 >= row) && (d < col || d + 3 >= col) || row == col;
        });
    }
}
...
Node::Node(uint32_t doFIndex, float mass, float damping, bool fixed, Eigen::Vector3f position) :
    _dofIndex(doFIndex),
    _mass(mass),
    _damping(damping),
    _fixed(fixed),
