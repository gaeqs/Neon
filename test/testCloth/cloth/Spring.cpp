...
Spring::Spring(uint32_t a, uint32_t b, SpringType type,
...
void Spring::updateState(const std::vector<Node>& nodes) {
    _direction = nodes[_nodeA].getPositionVector()
                 - nodes[_nodeB].getPositionVector();
    _length = _direction.norm();
    _direction /= _length;
    _velocityBA = nodes[_nodeA].getVelocityVector()
                  - nodes[_nodeB].getVelocityVector();
}
...
void Spring::getForceJacobian(
    ...
    auto ka = first + second;
    auto da = _damping * _stiffness * uM;
