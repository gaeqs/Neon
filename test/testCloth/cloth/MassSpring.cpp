...
void MassSpring::generateSprings(std::vector<uint32_t>& triangles,
                                 float dampingBeta,
                                 float stiffnessStretch,
                                 float stiffnessBend) {
...
void MassSpring::getPosition(Eigen::VectorXf& vector) const {
    for (const auto& item: _nodes) item.getPosition(vector);
}

void MassSpring::setPosition(const Eigen::VectorXf& vector) {
    for (auto& item: _nodes) item.setPosition(vector);
    for (auto& item: _springs) item.updateState(_nodes);
}

void MassSpring::getVelocity(Eigen::VectorXf& vector) const {
    for (const auto& item: _nodes) item.getVelocity(vector);
}

void MassSpring::setVelocity(const Eigen::VectorXf& vector) {
    for (auto& item: _nodes) item.setVelocity(vector);
}

void MassSpring::getForce(Eigen::VectorXf& vector) const {
    for (const auto& item: _nodes) item.getForce(vector);
    for (const auto& item: _springs) item.getForce(vector);
}

void MassSpring::getForceJacobian(
        std::vector<Eigen::Triplet<float>>& dFdx,
        std::vector<Eigen::Triplet<float>>& dFdv) const {
    for (const auto& item: _nodes) item.getForceJacobian(dFdx, dFdv);
    for (const auto& item: _springs) item.getForceJacobian(dFdx, dFdv);
}

void MassSpring::getMass(std::vector<Eigen::Triplet<float>>& mass) const {
    for (const auto& item: _nodes) item.getMass(mass);
}

void MassSpring::getInverseMass
        (std::vector<Eigen::Triplet<float>>& mass) const {
    for (const auto& item: _nodes) item.getInverseMass(mass);

}

void MassSpring::fixVector(Eigen::VectorXf& vector) const {
    for (const auto& item: _nodes) item.fixVector(vector);
}

void MassSpring::fixMatrix(Eigen::SparseMatrix<float>& matrix) const {
    for (const auto& item: _nodes) item.fixMatrix(matrix);
}
