...
void GlobalParametersUpdaterComponent::onUpdate(float deltaTime)
{
    ...
    Matrices matrices{camera.getView(), camera.getViewProjection(), camera.getFrustum().getInverseProjection(),
...
{
    ImGui::DragFloat(imGuiUId("Timestamp").c_str(), &_timestamp, 0.001f, 0.0f, 1.0f);
}
