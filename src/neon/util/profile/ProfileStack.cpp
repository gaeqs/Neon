...
namespace neon
{
    ...
    std::optional<std::shared_ptr<ProfileStack>> ProfileStack::getChild(const std::string& name) const
    {
    ...
    std::shared_ptr<ProfileStack> ProfileStack::getOrCreateChild(const std::string& name)
    {
        auto it = _children.find(name);
        if (it == _children.end()) {
    ...
    const std::deque<ProfileStack::ProfileDuration>& ProfileStack::getDurations() const
    {
        return _durations;
    }
    ...
    const std::unordered_map<std::string, std::shared_ptr<ProfileStack>>& ProfileStack::getChildren() const
    {
        return _children;
    }
} // namespace neon