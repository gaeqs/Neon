...
namespace neon
{
    const IdentifiableWrapper<GameObject>& GameObjectExplorerComponent::getTarget() const
    {
        return _target;
    }

    void GameObjectExplorerComponent::setTarget(const IdentifiableWrapper<GameObject>& target)
    {
        _target = target;
    }
    ...
    void GameObjectExplorerComponent::drawTransformSection() const
    {
        ...
        auto euler = rush::degrees(_target->getTransform().getRotation().euler());
    ...
    void GameObjectExplorerComponent::drawComponentsSection() const
    {
        ...
        for (const auto& component : list) {
            ...
            if (ImGui::BeginPopupContextItem(component->imGuiUId("component_context").c_str())) {
            ...
            if (openComponent && component.isValid()) {
                ...
                if (ImGui::Checkbox(component->imGuiUId("Enabled").c_str(), &enabled)) {
