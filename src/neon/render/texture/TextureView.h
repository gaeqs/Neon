...
namespace neon
{
    ...
    {
        ImageViewCreateInfo _info;
        std::shared_ptr<Texture> _texture;

      public:
        TextureView(std::string name, ImageViewCreateInfo info, std::shared_ptr<Texture> texture);

        [[nodiscard]] const std::shared_ptr<Texture>& getTexture() const;

        [[nodiscard]] const ImageViewCreateInfo& getInfo() const;
