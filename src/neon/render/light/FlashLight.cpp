...
namespace neon
{
    ...
    void FlashLight::onLateUpdate(float deltaTime)
    {
        ...
        _graphicComponent->uploadData(0, Data{_diffuseColor * _radiance, _specularColor * _radiance, position,
                                              direction, _constantAttenuation, _linearAttenuation,
                                              _quadraticAttenuation, _cutOff, _outerCutOff});
    }
    ...
    {
        float att = _constantAttenuation + _linearAttenuation * distance + _quadraticAttenuation * distance * distance;
    ...
    void FlashLight::drawEditor()
    {
        ...
        ImGui::SameLine();
        ImGui::DragFloat(imGuiUId("##constant_attenuation").c_str(), &_constantAttenuation, 0.05f);

        ImGui::AlignTextToFramePadding();
        ImGui::Text("Linear attenuation:");
        ImGui::SameLine();
        ImGui::DragFloat(imGuiUId("##linear_attenuation").c_str(), &_linearAttenuation, 0.05f);

        ImGui::AlignTextToFramePadding();
        ImGui::Text("Quadratic attenuation:");
        ImGui::SameLine();
        ImGui::DragFloat(imGuiUId("##quadratic_attenuation").c_str(), &_quadraticAttenuation, 0.05f);

        if (ImPlot::BeginPlot(imGuiUId("Attenuation / Distance").c_str(), ImVec2(-1, 200))) {
            ...
            ImPlot::PlotLine(imGuiUId("##attenuation").c_str(), values, i, 100.0 / MAX_VALUES);
            ImPlot::EndPlot();
        }
        ...
        ImGui::Text("Cut off:");
        ImGui::SameLine();
        if (ImGui::DragFloat(imGuiUId("##cut_off").c_str(), &cutOffAngle, 0.5f, 0.0f, 179.9f)) {
        ...
        ImGui::SameLine();
        if (ImGui::DragFloat(imGuiUId("##outer_cut_off").c_str(), &outerCutOffAngle, 0.5f, 0.0f, 179.9f)) {
