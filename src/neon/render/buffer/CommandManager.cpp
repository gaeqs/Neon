...
namespace neon
{
    ...
    CommandPoolHolder::CommandPoolHolder(CommandManager* manager, std::thread::id threadId) :
        _manager(manager),
        _threadId(threadId),
        _valid(true)
    {
    ...
    CommandPoolHolder& CommandPoolHolder::operator=(CommandPoolHolder&& move) noexcept
    {
        if (this == &move) {
            return *this; // SAME!
        }
