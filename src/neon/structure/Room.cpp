...
namespace neon
{
    ...
    void Room::destroyComponentLater(IdentifiableWrapper<neon::Component> component)
    {
    ...
    void Room::forEachGameObject(std::function<void(const GameObject*)> consumer) const
    {
        _gameObjects.forEach(std::move(consumer));
    }
        ...
        for (auto& jsonEntry : entries) {
            ...
            if (auto& transform = jsonEntry["transform"]; transform.is_object()) {
                ...
                if (position.is_object()) {
                    ...
                    if (auto& x = position["x"]; x.is_number()) {
                        result[0] = x.get<float>();
                    }
                    if (auto& y = position["y"]; y.is_number()) {
                        result[1] = y.get<float>();
                    }
                    if (auto& z = position["z"]; z.is_number()) {
                ...
                if (rotation.is_object()) {
                    ...
                    if (auto& x = rotation["x"]; x.is_number()) {
                        result[0] = x.get<float>();
                    }
                    if (auto& y = rotation["y"]; y.is_number()) {
                        result[1] = y.get<float>();
                    }
                    if (auto& z = rotation["z"]; z.is_number()) {
                        result[2] = z.get<float>();
                    }
            ...
            for (auto& jsonComponent : components) {
                ...
                auto type = ComponentRegister::instance().getEntry(jsonType.get<std::string>());
                if (!type.has_value() || !type.value().jsonCreator.has_value()) {
                    continue;
                }
    ...
    {
        DEBUG_PROFILE(getApplication()->getProfiler(), onCursorMove);
        _components.invokeScrollEvent(getApplication()->getProfiler(), event);
    }
    ...
    void Room::preDraw()
    {
        ...
        }
        {
            ...
            _application->getRender()->getGlobalUniformBuffer().prepareForFrame(cb);
