...
namespace neon {
    ...
    std::optional<rush::Vec3f> TextEffect::toRGBFromMetadata() const {
        if (metadata[0] == 5) {
            ...
            if (code < 232) {
                ...
                uint32_t g = code / 6 % 6;
                uint32_t r = code / 36;
                return {{r * 255 / 6, g * 255 / 6, b * 255 / 6}};
            }
        ...
        if (metadata[0] == 2) {
            return {{metadata[1], metadata[2], metadata[3]}};
        }
        return {};
    }
    ...
    TextEffect TextEffect::foregroundRGB(uint8_t red,
                                         uint8_t green,
                                         uint8_t blue) {
        return TextEffect(38, {2, red, green, blue});
    }

    TextEffect TextEffect::backgroundRGB(uint8_t red,
                                         uint8_t green,
                                         uint8_t blue) {
