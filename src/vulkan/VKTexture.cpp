...
{
    VKTexture::VKTexture(Application* application, const void* data, const TextureCreateInfo& dummyInfo) :
        ...
        _stagingBuffer(std::make_unique<SimpleBuffer>(
            _vkApplication, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, data,
            vc::pixelSize(dummyInfo.image.format) * _width * _height * _depth * _layers)),
        ...
        _externalDirtyFlag(1)
    {
        ...
        auto pair = vulkan_util::createImage(_vkApplication->getDevice()->getRaw(),
                                             _vkApplication->getPhysicalDevice().getRaw(), createInfo.image,
        ...
        //
        {
            ...
            vulkan_util::transitionImageLayout(_image, vkFormat, VK_IMAGE_LAYOUT_UNDEFINED,
        ...
        _imageView = vulkan_util::createImageView(_vkApplication->getDevice()->getRaw(), _image, vkFormat,
        ...
        VkSamplerCreateInfo samplerInfo = vc::vkSamplerCreateInfo(
            createInfo.sampler, static_cast<float>(createInfo.image.mipmaps), properties.limits.maxSamplerAnisotropy);

        if (vkCreateSampler(_vkApplication->getDevice()->getRaw(), &samplerInfo, nullptr, &_sampler) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create texture sampler!");
        }
    }

    VKTexture::VKTexture(Application* application, TextureFormat format, VkImage image, VkDeviceMemory memory,
        ...
        _externalDirtyFlag(1)
    {
        ...
        VkSamplerCreateInfo samplerInfo =
            vc::vkSamplerCreateInfo(sampler, static_cast<float>(_mipmapLevels), properties.limits.maxSamplerAnisotropy);
    ...
    void VKTexture::changeExternalImageView(int32_t width, int32_t height, VkImage image, VkDeviceMemory memory,
    ...
    {
        if (_external) {
            logger.error(MessageBuilder().group("vulkan").print("Couldn't update data of a texture"
