...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, VkPhysicalDeviceType value)
{
    const char* text;
    switch (value) {
...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, uint32_t value)
{
...
void neon::vulkan::VulkanInfoCompontent::printPropertyHex(const char* title, uint32_t value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, int32_t value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, uint64_t value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, bool value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::printPropertyBool(const char* title, VkBool32 value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, float value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, const char* value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::printProperty(const char* title, const std::string& value)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::Text(title);
    ImGui::SameLine(300 + window->DC.Indent.x);
...
void neon::vulkan::VulkanInfoCompontent::defaultPhysicalDeviceLimits() const
{
    ...
    printProperty("Max Image Dimensions",
                  std::format("{} * {} * {}", l.maxImageDimension1D, l.maxImageDimension2D, l.maxImageDimension3D));
    ...
    printPropertyHex("Storage Image Sample Counts", l.storageImageSampleCounts);
    ...
    printProperty("Max Combined Distances", l.maxCombinedClipAndCullDistances);
    printProperty("Discrete Queue Priorities", l.discreteQueuePriorities);

    printProperty("Point Size Range", std::format("{} - {}", l.pointSizeRange[0], l.pointSizeRange[1]));

    printProperty("Line Width Range", std::format("{} - {}", l.lineWidthRange[0], l.lineWidthRange[1]));
    ...
    printProperty("Standard Sample Locations", static_cast<bool>(l.standardSampleLocations));
    ...
    printProperty("  Max Uniform Buffers", l.maxPerStageDescriptorUniformBuffers);
    printProperty("  Max Storage Buffers", l.maxPerStageDescriptorStorageBuffers);
    printProperty("  Max Sampled Images", l.maxPerStageDescriptorSampledImages);
    printProperty("  Max Storage Images", l.maxPerStageDescriptorStorageImages);
    printProperty("  Max Input Attachments", l.maxPerStageDescriptorInputAttachments);

    ImGui::NewLine();
    ImGui::Text("Per Descriptor Set:");
    printProperty("  Max Samplers", l.maxDescriptorSetSamplers);
    printProperty("  Max Uniform Buffers", l.maxDescriptorSetUniformBuffers);
    printProperty("  Max Uniform Buffers (Dyn)", l.maxDescriptorSetUniformBuffersDynamic);
    printProperty("  Max Storage Buffers", l.maxDescriptorSetStorageBuffers);
    printProperty("  Max Storage Buffers (Dyn)", l.maxDescriptorSetStorageBuffersDynamic);
    ...
    printProperty("  Max Input Attachments", l.maxDescriptorSetInputAttachments);
    ...
    printProperty("  Max Input Att. Offset", l.maxVertexInputAttributeOffset);
    printProperty("  Max Input Binding Stride", l.maxVertexInputBindingStride);
    printProperty("  Max Output Components", l.maxVertexOutputComponents);

    ImGui::NewLine();
    ImGui::Text("Tessellation Control:");
    printProperty("  Max Per Vertex In. Comp.", l.maxTessellationControlPerVertexInputComponents);
    printProperty("  Max Per Vertex Out. Comp.", l.maxTessellationControlPerVertexOutputComponents);
    printProperty("  Max Per Patch Out. Comp.", l.maxTessellationControlPerPatchOutputComponents);
    printProperty("  Max Total Output Comp.", l.maxTessellationControlTotalOutputComponents);

    ImGui::NewLine();
    ImGui::Text("Tessellation Evaluation:");
    printProperty("  Max Input Components", l.maxTessellationEvaluationInputComponents);
    printProperty("  Max Output Components", l.maxTessellationEvaluationOutputComponents);
    ...
    printProperty("  Max Total Output Comp.", l.maxGeometryTotalOutputComponents);
    ...
    printProperty("  Max Output Components", l.maxFragmentOutputAttachments);
    printProperty("  Max Dual Src Attachments", l.maxFragmentDualSrcAttachments);
    printProperty("  Max Combined Out. Res.", l.maxFragmentCombinedOutputResources);

    ImGui::NewLine();
    ImGui::Text("Compute Shader:");
    printProperty("  Max Shared Memory Size", l.maxComputeSharedMemorySize);
    printProperty("  Max Work Group Count", std::format("{} * {} * {}", l.maxComputeWorkGroupCount[0],
                                                        l.maxComputeWorkGroupCount[1], l.maxComputeWorkGroupCount[2]));
    printProperty("  Max Work Group Invoc.", l.maxComputeWorkGroupInvocations);
    printProperty("  Max Work Group Size", std::format("{} * {} * {}", l.maxComputeWorkGroupSize[0],
                                                       l.maxComputeWorkGroupSize[1], l.maxComputeWorkGroupSize[2]));

    ImGui::NewLine();
    ImGui::Text("Viewport:");
    printProperty("  Max Amount", l.maxViewports);
    printProperty("  Max Dimension", std::format("{} * {}", l.maxViewportDimensions[0], l.maxViewportDimensions[1]));
    printProperty("  Bounds Range", std::format("{} - {}", l.viewportBoundsRange[0], l.viewportBoundsRange[1]));
    ...
    printProperty("  Uniform Buffer Offset", l.minUniformBufferOffsetAlignment);
    printProperty("  Storage Buffer Offset", l.minStorageBufferOffsetAlignment);
    ...
    printProperty("  Sub Pixel Offset Bits", l.subPixelInterpolationOffsetBits);
    ...
    printPropertyHex("  Color Sample Counts", l.framebufferColorSampleCounts);
    printPropertyHex("  Depth Sample Counts", l.framebufferDepthSampleCounts);
    printPropertyHex("  Stencil Sample Counts", l.framebufferStencilSampleCounts);
    printPropertyHex("  No Attach. Sample Counts", l.framebufferNoAttachmentsSampleCounts);

    ImGui::NewLine();
    ImGui::Text("Sampled Image:");
    printPropertyHex("  Color Sample Counts", l.sampledImageColorSampleCounts);
    printPropertyHex("  Integer Sample Counts", l.sampledImageIntegerSampleCounts);
    printPropertyHex("  Depth Sample Counts", l.sampledImageDepthSampleCounts);
    printPropertyHex("  Stencil Sample Counts", l.sampledImageStencilSampleCounts);

    ImGui::NewLine();
    ImGui::Text("Timestamp:");
    printProperty("  Compute And Graphics", static_cast<bool>(l.timestampComputeAndGraphics));
    printProperty("  Period", l.timestampPeriod);

    ImGui::NewLine();
    ImGui::Text("Optimal buffer:");
    printProperty("  Copy Offset Alignment", l.optimalBufferCopyOffsetAlignment);
    printProperty("  Copy Row Pitch Alighnment", l.optimalBufferCopyRowPitchAlignment);
}

void neon::vulkan::VulkanInfoCompontent::meshShaderPhysicalDeviceProperties() const
{
    auto& l = _meshShaderProperties;

    ImGui::Text("Task Work Group:");
    printProperty("  Max Total Count", l.maxTaskWorkGroupTotalCount);
    printProperty("  Max Count", std::format("{} * {} * {}", l.maxTaskWorkGroupCount[0], l.maxTaskWorkGroupCount[1],
    ...
    printProperty("Max Task Payload And Shared Memory Size", l.maxTaskPayloadAndSharedMemorySize);

    ImGui::Text("Mesh Work Group:");
    printProperty("  Max Total Count", l.maxMeshWorkGroupTotalCount);
    printProperty("  Max Count", std::format("{} * {} * {}", l.maxMeshWorkGroupCount[0], l.maxMeshWorkGroupCount[1],
    ...
    printProperty("Max Mesh Payload And Shared Memory Size", l.maxMeshPayloadAndSharedMemorySize);
...
void neon::vulkan::VulkanInfoCompontent::physicalDeviceFeatures() const
{
    if (ImGui::CollapsingHeader("Vulkan 1.0")) {
        ...
        printPropertyBool("Draw Indirect First Instance", f.drawIndirectFirstInstance);
        ...
        printPropertyBool("Texture Compression ASTC_LDR", f.textureCompressionASTC_LDR);
        printPropertyBool("Texture Compression ASTC_LDR", f.textureCompressionASTC_LDR);
        ...
        printPropertyBool("Pipeline Statistics Query", f.pipelineStatisticsQuery);
        printPropertyBool("Vertex Pipeline Stores And Atomics", f.vertexPipelineStoresAndAtomics);
        printPropertyBool("Fragment Stores And Atomics", f.fragmentStoresAndAtomics);
        printPropertyBool("Shader Tess./Geom. Point Size", f.shaderTessellationAndGeometryPointSize);
        printPropertyBool("Shader Image Gather Extended", f.shaderImageGatherExtended);
        printPropertyBool("Shader Storage Image Extended Formats", f.shaderStorageImageExtendedFormats);
        printPropertyBool("Shader Storage Image Multisample", f.shaderStorageImageMultisample);
        printPropertyBool("Shader Storage Image Read Without Format", f.shaderStorageImageReadWithoutFormat);
        printPropertyBool("Shader Storage Image Write Without Format", f.shaderStorageImageWriteWithoutFormat);
        printPropertyBool("Shader Uniform Buffer Array Dyn. Indexing", f.shaderUniformBufferArrayDynamicIndexing);
        printPropertyBool("Shader Sampled Image Array Dyn. Indexing", f.shaderSampledImageArrayDynamicIndexing);
        printPropertyBool("Shader Storage Buffer Array Dyn. Indexing", f.shaderStorageBufferArrayDynamicIndexing);
        printPropertyBool("Shader Storage Image Array Dyn. Indexing", f.shaderStorageImageArrayDynamicIndexing);
        ...
        printPropertyBool("Shader Resource Residency", f.shaderResourceResidency);
        ...
        printPropertyBool("Sparse Residency 2 Samples", f.sparseResidency2Samples);
        printPropertyBool("Sparse Residency 4 Samples", f.sparseResidency4Samples);
        printPropertyBool("Sparse Residency 8 Samples", f.sparseResidency8Samples);
        printPropertyBool("Sparse Residency 16 Samples", f.sparseResidency16Samples);
        printPropertyBool("Sparse Residency Aliased", f.sparseResidencyAliased);
        printPropertyBool("Variable Multisample Rate", f.variableMultisampleRate);
    ...
    if (ImGui::CollapsingHeader("Vulkan 1.1")) {
        ImGui::TreePush("");
        const auto& o = _features->findFeature<VkPhysicalDeviceVulkan11Features>(
            VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_1_FEATURES);
        if (o.has_value()) {
            auto* f = o.value();
            printPropertyBool("Storage Buffer 16 Bit Access", f->storageBuffer16BitAccess);
            printPropertyBool("Uniform/Storage Buffer 16 Bit Access", f->uniformAndStorageBuffer16BitAccess);
            ...
            printPropertyBool("Multiview Geometry Shader", f->multiviewGeometryShader);
            printPropertyBool("Multiview Tessellation Shader", f->multiviewTessellationShader);
            printPropertyBool("Variable Pointers Storage Buffer", f->variablePointersStorageBuffer);
    ...
    if (ImGui::CollapsingHeader("Vulkan 1.2")) {
        ImGui::TreePush("");
        const auto& o = _features->findFeature<VkPhysicalDeviceVulkan12Features>(
            VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES);
        if (o.has_value()) {
            auto* f = o.value();
            printPropertyBool("Sampler Mirror Clamp To Edge", f->samplerMirrorClampToEdge);
            printPropertyBool("Draw Indirect Count", f->drawIndirectCount);
            printPropertyBool("Storage Buffer 8 Bit Access", f->storageBuffer8BitAccess);
            printPropertyBool("Uniform/Storage Buffer 8 Bit Access", f->uniformAndStorageBuffer8BitAccess);
            printPropertyBool("Storage Push Constant 8", f->storagePushConstant8);
            printPropertyBool("Shader Buffer Int64 Atomics", f->shaderBufferInt64Atomics);
            printPropertyBool("Shader Shared Int64 Atomics", f->shaderSharedInt64Atomics);
            ...
            printPropertyBool("Shader Input Att. Array Dyn. Indexing", f->shaderInputAttachmentArrayDynamicIndexing);
            ...
            printPropertyBool("S. Sampled Image Array Non Uniform Ind.", f->shaderSampledImageArrayNonUniformIndexing);
            printPropertyBool("S. Sorage Buffer Array Non Uniform Ind.", f->shaderStorageBufferArrayNonUniformIndexing);
            printPropertyBool("S. Sorage Image Array Non Uniform Ind.", f->shaderStorageImageArrayNonUniformIndexing);
            ...
            printPropertyBool("Desc. Binding Upate Unused While Pending", f->descriptorBindingUpdateUnusedWhilePending);
            printPropertyBool("Desc. Binding Partially Bound", f->descriptorBindingPartiallyBound);
            printPropertyBool("Desc. Binding Variable Descriptor Count", f->descriptorBindingVariableDescriptorCount);
            ...
            printPropertyBool("Uniform Buffer Standard Layout", f->uniformBufferStandardLayout);
            printPropertyBool("Shader Subgroup Extended Types", f->shaderSubgroupExtendedTypes);
            printPropertyBool("Separated Depth Stencil Layouts", f->separateDepthStencilLayouts);
            ...
            printPropertyBool("Buffer Device Address Capture Replay", f->bufferDeviceAddressCaptureReplay);
            printPropertyBool("Buffer Device Address Multi Device", f->bufferDeviceAddressMultiDevice);
            printPropertyBool("Vulkan Memory Model", f->vulkanMemoryModel);
            printPropertyBool("Vulkan Memory Model Device Scope", f->vulkanMemoryModelDeviceScope);
            printPropertyBool("Vulkan Mem. Model Avail. Visib. Chains",
                              f->vulkanMemoryModelAvailabilityVisibilityChains);
            printPropertyBool("Shader Output Viewport Index", f->shaderOutputViewportIndex);
            printPropertyBool("Shader Output Layer", f->shaderOutputLayer);
            printPropertyBool("Subgroup Broadcast Dynamic ID", f->subgroupBroadcastDynamicId);
        }
        ImGui::TreePop();
    }
    ...
    if (ImGui::CollapsingHeader("Vulkan 1.3")) {
        ImGui::TreePush("");
        const auto& o = _features->findFeature<VkPhysicalDeviceVulkan13Features>(
    ...
    if (ImGui::CollapsingHeader("Mesh Shader")) {
        ImGui::TreePush("");
        const auto& o = _features->findFeature<VkPhysicalDeviceMeshShaderFeaturesEXT>(
...
void neon::vulkan::VulkanInfoCompontent::onStart()
{
    ...
    vkGetPhysicalDeviceProperties2(application->getPhysicalDevice().getRaw(), &prop);
}
