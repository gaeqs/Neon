...
{
    VKConcurrentInstanceData::VKConcurrentInstanceData(Application* application, const ModelCreateInfo& info) :
        _vkApplication(dynamic_cast<AbstractVKApplication*>(application->getImplementation()))
    {
        ...
        for (const auto& size : info.instanceSizes) {
            ...
            if (size == 0) {
                ...
                _buffers.push_back(
                    std::make_unique<StagingBuffer>(_vkApplication, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, bufferSize));
            }
        }
    }

    const std::vector<std::unique_ptr<Buffer>>& VKConcurrentInstanceData::getBuffers() const
    {
        return _buffers;
    }

    void VKConcurrentInstanceData::flush(const CommandBuffer* command, size_t index, size_t instanceSize, void* data,
                                         Range<uint32_t> changeRange) const
    {
        ...
        memcpy(map.value()->raw(), static_cast<char*>(data) + changeRange.getFrom(), changeRange.size());
    }
} // namespace neon::vulkan
