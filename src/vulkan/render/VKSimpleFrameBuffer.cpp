...
{
    void VKSimpleFrameBuffer::createImages(std::shared_ptr<Texture> overrideDepth)
    {
        _images.clear();
        ...
        for (const auto& createInfo : _createInfos) {
            ...
            auto [image, memory] = vulkan_util::createImage(_vkApplication->getDevice()->getRaw(),
                                                            _vkApplication->getPhysicalDevice().getRaw(), info,
                                                            createInfo.imageView.viewType);

            auto view = vulkan_util::createImageView(_vkApplication->getDevice()->getRaw(), image,
                                                     conversions::vkFormat(info.format), VK_IMAGE_ASPECT_COLOR_BIT,
                                                     createInfo.imageView);

            SimpleFrameBufferSlot slot;
            slot.image = SimpleFrameBufferImage(createInfo.name, false, info, createInfo.sampler, image, memory,
                                                VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, view, nullptr, nullptr);

            if (_hasMultisampling) {
                // Create resolve texture
                info.samples = SamplesPerTexel::COUNT_1;
                auto [resolveImage, resolveMemory] = vulkan_util::createImage(
                    _vkApplication->getDevice()->getRaw(), _vkApplication->getPhysicalDevice().getRaw(), info,
                    createInfo.imageView.viewType);

                auto resolveView = vulkan_util::createImageView(_vkApplication->getDevice()->getRaw(), resolveImage,
                                                                conversions::vkFormat(info.format),
                                                                VK_IMAGE_ASPECT_COLOR_BIT, createInfo.imageView);

                slot.resolveImage = SimpleFrameBufferImage(
                    createInfo.resolveName, false, info, createInfo.sampler, resolveImage, resolveMemory,
                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, resolveView, nullptr, nullptr);
            }
        ...
        if (overrideDepth == nullptr) {
            ...
            auto [image, memory] =
                vulkan_util::createImage(_vkApplication->getDevice()->getRaw(),
                                         _vkApplication->getPhysicalDevice().getRaw(), info, TextureViewType::NORMAL_2D,
            ...
            SimpleFrameBufferSlot slot{
                {_depthName, true, info, depthSampler, image, memory, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
            ...
            SimpleFrameBufferSlot slot{
                {_depthName, true, info, depthSampler, impl.getImage(), impl.getMemory(),
                 VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL, impl.getImageView(), nullptr, overrideDepth}
            };
    ...
    void VKSimpleFrameBuffer::createFrameBuffer()
    {
        ...
        if (vkCreateFramebuffer(_vkApplication->getDevice()->getRaw(), &framebufferInfo, nullptr, &_frameBuffer) !=
            VK_SUCCESS) {
    ...
    void VKSimpleFrameBuffer::createOutputs(Application* application)
    {
        ...
        for (auto& item : _images) {
            ...
            if (item.image.overrideTexture != nullptr) {
                ...
                output.texture =
                    std::make_shared<Texture>(application, item.image.name.value_or(std::to_string(i)),
                                              item.image.image, item.image.memory, item.image.view, item.image.layout,
                                              static_cast<int32_t>(_extent.width), static_cast<int32_t>(_extent.height),
                                              1, item.image.info.format, item.image.info.samples, item.image.sampler);
            }
            ...
            if (item.resolveImage.has_value()) {
                if (item.resolveImage->overrideTexture != nullptr) {
                    ...
                    output.resolvedTexture = std::make_shared<Texture>(
                        application, item.resolveImage->name.value_or(std::to_string(i)), item.resolveImage->image,
                        item.resolveImage->memory, item.resolveImage->view, item.resolveImage->layout,
                        static_cast<int32_t>(_extent.width), static_cast<int32_t>(_extent.height), 1,
                        item.resolveImage->info.format, item.resolveImage->info.samples, item.resolveImage->sampler);
                }
            } else {
    ...
    VKSimpleFrameBuffer::VKSimpleFrameBuffer(Application* application,
                                             const std::vector<FrameBufferTextureCreateInfo>& textureInfos,
                                             std::pair<uint32_t, uint32_t> extent, bool depth,
        ...
        _renderPass(application, conversions::vkFormat(textureInfos), conversions::vkSampleCountFlagBits(textureInfos),
                    depth, false, depthSamples != SamplesPerTexel::COUNT_1, _vkApplication->getDepthImageFormat(),
    ...
    VKSimpleFrameBuffer::VKSimpleFrameBuffer(Application* application,
        ...
        _extent({extent.first, extent.second}),
        _renderPass(application, conversions::vkFormat(textureInfos), conversions::vkSampleCountFlagBits(textureInfos),
                    depthTexture != nullptr, false, true, conversions::vkFormat(depthTexture->getFormat()),
    ...
    void VKSimpleFrameBuffer::recreate(std::pair<uint32_t, uint32_t> size)
    {
        ...
        for (uint32_t i = 0; i < _images.size(); ++i) {
            ...
            texture->getImplementation().changeExternalImageView(static_cast<int32_t>(_extent.width),
                                                                 static_cast<int32_t>(_extent.height), item.image.image,
                                                                 item.image.memory, item.image.view);

            if (auto& resolved = _outputs[i].resolvedTexture; resolved != texture) {
                resolved->getImplementation().changeExternalImageView(
                    static_cast<int32_t>(_extent.width), static_cast<int32_t>(_extent.height), item.resolveImage->image,
                    item.resolveImage->memory, item.resolveImage->view);
            }
        }
    }
    ...
    ImTextureID VKSimpleFrameBuffer::getImGuiDescriptor(uint32_t index)
    {
        ...
        if (target.imGuiDescriptor == nullptr) {
            ...
            target.imGuiDescriptor = ImGui_ImplVulkan_AddTexture(texture->getImplementation().getSampler(),
    ...
    std::pair<uint32_t, uint32_t> VKSimpleFrameBuffer::defaultRecreationParameters() const
    {
        auto extent = _vkApplication->getSwapChainExtent();
